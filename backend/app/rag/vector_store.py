...
from app.model.factory import get_embedding_model
...
class VectorStoreService:
    ...
    def load_documents(self) -> dict[str, int]:
        """增量导入知识目录中的 TXT/PDF；相同文件不会重复生成向量。"""
        ...
        if not data_path.is_dir():
            raise RuntimeError(f"RAG 知识目录不存在: {data_path}")
    ...
    def similarity_search(
        ...
        min_score = float(chroma_config.get("min_relevance_score", 0.0))
        return [
            (document, score)
